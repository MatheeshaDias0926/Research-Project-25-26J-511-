...
@app.put("/bus-route", response_model=BusRouteUpdateResponse)
def update_bus_route(payload: BusRouteUpdateRequest, x_api_key: str | None = Header(default=None)):
    require_key(x_api_key)

    bus_id = payload.busId.strip()
    route_no = payload.routeNo.strip()

    if not store.get(route_no):
        raise HTTPException(status_code=404, detail=f"Route {route_no} not found")

    previous_route_no = store.set_bus_route(bus_id, route_no)

    return {
        "ok": True,
        "busId": bus_id,
        "previousRouteNo": previous_route_no,
        "routeNo": route_no,
    }
