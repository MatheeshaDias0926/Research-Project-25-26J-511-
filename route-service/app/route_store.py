...
class RouteStore:
    ...
    def _persist_bus_mapping(self):
        DATA.mkdir(parents=True, exist_ok=True)
        buses_path = DATA / "buses.json"
        payload = {
            bus_id: {"routeNo": route_no}
            for bus_id, route_no in sorted(self.bus_to_route.items())
            if route_no is not None
        }
        buses_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")

    def set_bus_route(self, bus_id: str, route_no: str):
        previous = self.bus_to_route.get(bus_id)
        self.bus_to_route[bus_id] = str(route_no)
        self._persist_bus_mapping()
        return previous

    def remove_bus_route(self, bus_id: str):
        previous = self.bus_to_route.pop(bus_id, None)
        self._persist_bus_mapping()
        return previous
