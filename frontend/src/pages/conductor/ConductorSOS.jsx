...
const ConductorSOS = () => {
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ background: "#fff", padding: 24, borderRadius: "var(--radius-lg)", border: "2px solid #ef4444", marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div>
              <label style={{ fontSize: "var(--text-sm)", fontWeight: 500, marginBottom: 4, display: "block" }}>Alert Type</label>
            ...
            <div>
              <label style={{ fontSize: "var(--text-sm)", fontWeight: 500, marginBottom: 4, display: "block" }}>Description</label>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              ...
              <button type="button" onClick={() => setShowForm(false)} style={{ padding: "12px 24px", background: "#e2e8f0", color: "var(--text-secondary)", border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 600 }}>
                Cancel
              </button>
            </div>
          </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {alerts.map((alert) => {
          ...
          return (
            <div key={alert._id} style={{ background: "#fff", border: "1px solid var(--border-light)", borderRadius: "var(--radius-lg)", padding: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <span style={{ fontWeight: 600, textTransform: "capitalize" }}>{alert.alertType}</span>
                    <span style={{ padding: "3px 10px", borderRadius: "var(--radius-lg)", fontSize: 12, fontWeight: 500, background: sc.bg, color: sc.color }}>{alert.status}</span>
                  </div>
                  {alert.description && <p style={{ color: "var(--text-muted)", fontSize: 13, marginTop: 4 }}>{alert.description}</p>}
                </div>
                <div style={{ fontSize: "var(--text-sm)", color: "var(--text-muted)", display: "flex", alignItems: "center", gap: 4 }}>
                  <Clock size={14} />
