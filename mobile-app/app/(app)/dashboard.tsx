...
import { getBusStatus, getPhysicsModel } from "../../src/api/bus";
...
export default function DashboardScreen() {
  ...
  // Fetch physics model result when bus status updates
  useEffect(() => {
    if (!data || !busId) return;
    // Prepare payload for physics model
    const payload = {
      busId,
      seated: status.seated || 0,
      standing: status.standing || 0,
    ...
    getPhysicsModel(payload)
      .then(setPhysicsResult)
      .catch((err) =>
        setPhysicsError(err?.response?.data?.message || err.message)
      )
      .finally(() => setPhysicsLoading(false));
  }, [data, busId]);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            router.push({ pathname: "/maintenance", params: { busId } })
          }
        >
      ...
      <ScrollView contentContainerStyle={styles.content}>
          ...
          <View style={[styles.alertCard, styles.dangerAlert]}>
            ...
            <View style={styles.alertTextContainer}>
              ...
              <Text style={styles.alertDesc}>
                Passenger on footboard! Stop immediately.
              </Text>
            </View>
          </View>
        )}
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.gpsText}>
            Lat: {status.gps?.lat?.toFixed(4) || "N/A"}, Lng:{" "}
            {status.gps?.lon?.toFixed(4) || "N/A"}
          </Text>
        </View>
        ...
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            <Text style={styles.cardTitle}>Physics Model</Text>
            <Ionicons name="analytics" size={20} color="#64748b" />
          </View>
          {physicsLoading ? (
            <Text>Loading physics model...</Text>
          ) : physicsError ? (
            <Text style={{ color: "red" }}>Error: {physicsError}</Text>
          ) : physicsResult ? (
            <View>
              {physicsResult.cog_height_m !== undefined && (
                <Text>
                  CoG Height: {physicsResult.cog_height_m.toFixed(2)} m
                </Text>
              )}
              {physicsResult.rollover_threshold_g !== undefined && (
                <Text>
                  Rollover Threshold:{" "}
                  {physicsResult.rollover_threshold_g.toFixed(2)} g
                </Text>
              )}
              {physicsResult.lateral_accel_g !== undefined && (
                <Text>
                  Lateral Accel: {physicsResult.lateral_accel_g.toFixed(2)} g
                </Text>
              )}
              {physicsResult.radius_m !== undefined && (
                <Text>Curve Radius: {physicsResult.radius_m.toFixed(1)} m</Text>
              )}
              {physicsResult.decision && (
                <Text>Decision: {physicsResult.decision}</Text>
              )}
              {physicsResult.stopping && (
                <>
                  <Text>
                    Stopping Distance:{" "}
                    {physicsResult.stopping.total_stopping_distance_m?.toFixed(
                      1
                    )}{" "}
                    m
                  </Text>
                  <Text>
                    Deceleration:{" "}
                    {physicsResult.stopping.deceleration_g?.toFixed(2)} g
                  </Text>
                </>
              )}
            </View>
          ) : (
            <Text>No physics data available.</Text>
          )}
        </View>
      </ScrollView>
    </SafeAreaView>
